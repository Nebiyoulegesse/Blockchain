...
class Blockchain:
    ...
    # proof of work
    # Has to be a problem that is hard to solve but easy to verify

    def proof_of_work(self, previous_proof):
        new_proof = 1
        check_proof = False
        while check_proof is False:
            # implementation of the SHA256 hashing function

            hash_opperation = hashlib.sha256(str(new_proof**2 - previous_proof**2).encode()).hexdigest()
            if hash_opperation[:4] == "0000":
                check_proof = True
            else:
                new_proof += 1
        return new_proof
    ...
    def is_chain_valid(self, chain):
        ...
        while block_index < len(chain):
            ...
            hash_opperation = hashlib.sha256(str(proof**2 - previous_proof**2).encode()).hexdigest()
            if hash_opperation[:4] != "0000":
                return False
...
# used to reward nodes who solve the hash


#Node adress at port 5000
...
def mine_block():
    ...
    blockchain.add_transaction(sender= node_address, receiver= "Pirate_2", amount= 1)
